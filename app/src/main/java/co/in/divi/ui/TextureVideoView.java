...
/**
 * Displays a video file. The TextureVideoView class can load images from various sources (such as resources or content
 ...
 * background.</em> In particular, it does not restore the current play state, play position or selected tracks.
 * Applications should save and restore these on their own in {@link android.app.Activity#onSaveInstanceState} and
 * {@link android.app.Activity#onRestoreInstanceState}.
 * <p>
 * Also note that the audio session id (from {@link #getAudioSessionId}) may change from its previously returned value
 ...
 * <ol>
 * <li>extends {@link android.view.TextureView} instead of a {@link android.view.SurfaceView} allowing proper view
 * animations</li>
...
public class TextureVideoView extends TextureView implements MediaPlayerControl {
	...
	private int								mSeekWhenPrepared;									// recording the seek
																								// position while
																								// preparing
	...
	MediaPlayer.OnPreparedListener					mPreparedListener			= new MediaPlayer.OnPreparedListener() {
																					public void onPrepared(MediaPlayer mp) {
																						...
																						if (mVideoWidth != 0 && mVideoHeight != 0) {
																							...
																							// We don't know the video
																							// size yet, but should
																							// start anyway.
																							// The video size might be
																							// reported to us later.
																							if (mTargetState == STATE_PLAYING) {
																								start();
																							}
																						}
	...
	private MediaPlayer.OnCompletionListener		mCompletionListener			= new MediaPlayer.OnCompletionListener() {
																					public void onCompletion(MediaPlayer mp) {
																						if (mCurrentState == STATE_PLAYBACK_COMPLETED) {
																							// sprylab bugfix: on some
																							// devices onCompletion is
																							// called twice
																							return;
																						}
	...
	private MediaPlayer.OnErrorListener				mErrorListener				= new MediaPlayer.OnErrorListener() {
																					public boolean onError(MediaPlayer mp,
																						 ...
																						 * dialog so the user knows that
																						 * something bad has happened.
																						 * Only try and pop up the
																						 * dialog if we're attached to a
																						 * window. When we're going away
																						 * and no longer have a window,
																						 * don't bother showing the user
																						 * an error.
																						 */
	 ...
	 * if the listener returned false, TextureVideoView will inform the user of any errors.
	 * 
	 * @param l
	 *            The callback that will be run
	 */
	...
	TextureView.SurfaceTextureListener	mSurfaceTextureListener	= new SurfaceTextureListener() {
																	...
																	public boolean onSurfaceTextureDestroyed(final SurfaceTexture surface) {
																		// after we return from this we can't use the
																		// surface any more
																		mSurface = null;
																		if (mMediaController != null)
																			mMediaController.hide();
