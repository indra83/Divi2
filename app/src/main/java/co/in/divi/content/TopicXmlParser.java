...
public class TopicXmlParser {
    ...
    public TopicNode getNodeFromXml(String title, File topicXmlFile, String relFilePath) {
        ...
        ArrayList<Topic.App> apps = new ArrayList<Topic.App>();
        ...
        try {
            ...
            topicData.apps = apps.toArray(new Topic.App[0]);
    ...
    private Topic.App fillApp(XmlPullParser parser) throws IOException, XmlPullParserException {
        if (LogConfig.DEBUG_XML)
            Log.d(TAG, "fillVM");
        parser.require(XmlPullParser.START_TAG, ns, TopicXmlTags.APP_TAG);
        String id = parser.getAttributeValue(ns, TopicXmlTags.ID_ATTRIBUTE);
        String src = parser.getAttributeValue(ns, TopicXmlTags.SRC_ATTRIBUTE);
//		String thumb = parser.getAttributeValue(ns, TopicXmlTags.App);

        Topic.App app = new Topic.App();
        app.id = id;
        app.src = src;
//		app.thumb = thumb;
        app.appPackage = parser.getAttributeValue(ns, TopicXmlTags.APP_PACKAGE_ATTRIBUTE);
        int verCode = -1;
        try {
            verCode = Integer.parseInt(parser.getAttributeValue(ns, TopicXmlTags.APP_VERSIONCODE_ATTRIBUTE));
        } catch (NumberFormatException nfe) {
            // default is -1;
        }
        app.appVersionCode = verCode;
        app.appActivityName = parser.getAttributeValue(ns, TopicXmlTags.APP_ACTIVITY_NAME_ATTRIBUTE);
//		app.showInApps = Boolean.parseBoolean(parser.getAttributeValue(ns, TopicXmlTags.VM_SHOW_IN_APPS_ATTRIBUTE));
        // get description
        while (parser.next() != XmlPullParser.END_TAG) {// VIDEO end tag
            if (parser.getEventType() != XmlPullParser.START_TAG) {
                continue;
            ...
            if (name.equals(TopicXmlTags.APP_DESCRIPTION_TAG)) {
                app.desc = readText(parser);
            } else if (name.equals(TopicXmlTags.APP_TITLE_TAG)) {
                app.title = readText(parser);
            } else {
                skip(parser);
            }
        }
        app.hideInToc = "true".equalsIgnoreCase(parser.getAttributeValue(ns, TopicXmlTags.HIDETOC_ATTRIBUTE));
        app.isPlaystoreApp = app.src == null || app.src.length() == 0 ? true : false;
        return app;
    }
